...
1. Install Rust. The repository pins the toolchain in `rust-toolchain.toml` (currently Rust 1.85), so `rustup` will automatically install the correct version when you run any cargo command in the workspace. Keep this version in sync with `rust-toolchain.toml` and the workspace `rust-version` in `Cargo.toml`.
