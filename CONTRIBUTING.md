...
1. Install Rust. The repository pins the toolchain in `rust-toolchain.toml` (currently Rust 1.85), matching the workspace `rust-version`; keep these versions in sync when updating the toolchain. `rustup` will automatically install the pinned version when you run any cargo command in the workspace.
