...
1. Install Rust. The repository pins the toolchain in `rust-toolchain.toml` (currently Rust 1.85, aligned with workspace `rust-version`), so `rustup` will automatically install the correct version when you run any cargo command in the workspace.
